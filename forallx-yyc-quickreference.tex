...
所有 $F$ 都是 $G$ & $\forall x(\atom{F}{x} \eif \atom{G}{x})$\\
...
\\
\multicolumn{2}{c}{\textsc{等词}}\\ \\
只有 $c$ 是 $G$ & $\forall x(\atom{G}{x} \eiff x=c)$\\
除 $c$ 之外的其他\\都是 $G$ & $\forall x(\enot\, {x = c} \eif \atom{G}{x} )$\\
除 $c$ 以外的一切\\都是 $G$ & $\forall x(\enot\, {x = c} \eiff \atom{G}{x} )$\\
...
一种表达“至多有 $n$ 个 $F$”的方法，是在“至少有 $n+1$ 个 $F$”的符号化前面加一个否定号。等价地，我们可以给出：
...
一种表达“恰好有 $n$ 个 $F$”的方法是，把上面的两种符号化合取起来，说“至少有 $n$ 个 $F$ 且至多有 $n$ 个 $F$”。以下等价公式更简短：
...
\end{fitchproof}
\begin{raggedright}
\noindent \metav{c} 不得出现在任何未解除的假设、$\exists \metav{x}\,\metav{A}(\ldots \metav{x} \ldots \metav{x}\ldots)$ 或 \metav{B} 中。\end{raggedright}
\end{multicols}
...
\begin{compactlist}
\item 该行必须出现在应用规则的那一行之前，但
\item 不得出现在应用规则行之前已经闭合的子证明内。
\end{compactlist}
...
\item 被引用的子证明不得位于某个在引用行处已经闭合的其他子证明内，并且
\item 被引用子证明的最后一行不得位于嵌套子证明内部。
\end{compactlist}
