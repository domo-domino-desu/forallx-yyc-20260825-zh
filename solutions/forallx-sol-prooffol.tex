...
\begin{compactlist}
\item \myanswer{\begin{fitchproof}
...
`$\forall x (\atom{F}{x} \eif \enot \atom{G}{x})$'. \\\myanswer{We
prove the four Figure I syllogisms; the rest are \emph{extremely}
similar.
\begin{compactlist}
\item \textbf{Barbara}
\begin{fitchproof}
...
\end{fitchproof}
\item\textbf{Celerant} is exactly as Barbara, replacing `$F$' with `$\enot F$' throughout.
\item\textbf{Ferio} 
\begin{fitchproof}
...
\end{fitchproof}
\item\textbf{Darii} is exactly as Ferio, replacing `$\enot F$' with `$F$' throughout.\end{compactlist}}
...
\begin{ekey}
\item[\text{论域}] all people
\item[\atom{L}{x,y}] \gap{x} likes \gap{y}
\end{ekey}
$\exists x \forall y(\forall z(\atom{L}{x,z} \eif \atom{L}{y,z}) \eif \atom{L}{x,y}) \therefore \exists x  \,\atom{L}{x,x}$
\begin{fitchproof}
...
\\\myanswer{Counter-interpretation: let the domain be the numbers $1$ and $2$. Let `$c$' name $1$. Let `$\atom{P}{x}$' be true of and only of $1$. Let `$\atom{Q}{x}$' be true of, and only of, $2$.}
\item $\forall x\forall y \forall z \atom{B}{x,y,z}, \forall x\,\atom{B}{x,x,x}$\hfill \myanswer{Not logically equivalent}
\\\myanswer{Counter-interpretation: let the domain be the numbers $1$ and $2$. Let `$\atom{B}{x,y,z}$' be true of, and only of, \ntuple{1,1,1} and \ntuple{2,2,2}.}
\item $\forall x\forall y\,\atom{D}{x,y}, \forall y\forall x\,\atom{D}{x,y}$ \hfill \myanswer{interderivable\begin{multicols}{2}
\begin{fitchproof}[arrayenv=tabular]
...
\end{tikzpicture}
\end{center}}
\item $\forall x (\atom{R}{c,a} \eiff \atom{R}{x,a}), \atom{R}{c,a} \eiff \forall x\,\atom{R}{x,a}$ \hfill \myanswer{Not logically equivalent}
...
\item $\forall x\forall y(\atom{R}{x,y} \eor \atom{R}{y,x}) \therefore \atom{R}{j,j}$ \hfill \myanswer{Valid
\begin{fitchproof}
...
\item $\exists x\exists y(\atom{R}{x,y} \eor \atom{R}{y,x}) \therefore \atom{R}{j,j}$ \hfill \myanswer{Invalid\\
...
Counter-interpretation: let the domain be the number $1$. Let `$\atom{P}{x}$' be true of nothing. Let `$\atom{Q}{x}$' be true of everything.}
...
\stepcounter{chapter} % Proofs with quantifiers
...
\problempart
Show that each pair of sentences is interderivable: %TB: Again, have to do some manual numbering to make the multicols have enough space.
...
\close
\have{Aab2}{\forall x\,\atom{A}{x} \eor \atom{B}{d}}\tnd{b-Aab, nb-Aab1}
\end{fitchproof}
\begin{fitchproof}
...
\noindent\myanswer{In one direction:
\begin{fitchproof}
...
\item[\texttherefore] $\exists x \exists y \bigl[\enot x = y \eand \forall z ((\atom{F}{z} \eor \atom{G}{z}) \eif (x = z \eor y = z))\bigr]$
\end{earg}}
...
\problempart
Offer proofs which justify the addition of the second and fourth CQ
rules as derived rules.

\noindent\myanswer{Justification for the second rule:
\begin{fitchproof}
...
\end{fitchproof}
The fourth rule is harder to justify.  Here is a proof that
...
\stepcounter{chapter} % Proofs and semantics