...
一阶逻辑的语言使用了真值函项逻辑的所有联结词。因此，一阶逻辑中的证明也将使用 \cref{ch.NDTFL} 中的所有基本规则和派生规则。我们也将沿用那里引入的证明论概念（特别是符号“$\proves$”）。不过，我们还需要一些新的基本规则来约束量词以及等号。

\section{全称消去}\label{s:forallElim}

从“所有东西都是~$F$”这一主张，你可以推断任何特定的对象都是~$F$。你随便指一个，它就是~$F$。所以下面这个推导应该是有效的：
...
我们这里通过去掉全称量词并把所有“$x$”的出现都替换为“$d$”得到了第~$2$行。我们也可以用任何其他名称做同样的替换。
...
此处记法在\cref{s:TruthFOL}中引入。关键在于，你可以得到全称量化公式的任何\emph{代入实例}：把被量化变元的每一个自由出现都替换为任何一个名称。

我们必须强调，（与所有消去规则一样）只有当全称量词是主逻辑算子时，你才能应用 $\forall$E 规则。因此下面这种操作是\emph{被禁止的}：

\begin{fitchproof}
	\hypo{a}{\forall x\,\atom{B}{x} \eif \atom{B}{k}}\PR
	\have{c}{\atom{B}{b} \eif \atom{B}{k}}\by{试图调用 $\forall$E 的错误尝试}{a}
\end{fitchproof}

这是不合法的，因为“$\forall x$”不是第~$1$行的主逻辑算子。（如果你需要复习为什么这类推理应该被禁止，请重读 \cref{s:MoreMonadic}。）

\section{存在引入}\label{s:existsIntro}

从“某个特定的东西是~$F$”这一主张，你可以推出有东西是~$F$。所以我们应该允许：
...
这里，我们把名称“$d$”替换为变元“$x$”，然后对其进行存在量化。同样地，我们也应该允许：
...
这里我们把名称“$a$”的\emph{所有}出现都替换为变元，然后进行了存在概括。但我们并不需要把名称的\emph{所有}出现都替换为变元：如果 Narcissus 爱他自己，那么存在某个爱 Narcissus 的人。所以我们也允许：
...
当公式 $\metav{A}$ 包含名称 $\metav{c}$ 时，我们可以通过写作“$\metav{A}(\ldots \metav{c} \ldots
\metav{c}\ldots)$”来强调这一点。我们将用“$\metav{A}(\ldots \metav{x} \ldots
\metav{c}\ldots)$”来表示通过将名称 \metav{c} 的\emph{部分或全部}出现替换为变元~\metav{x} 而得到的任何公式（前提是，无论在何处替换，$\metav{c}$ 都不出现在绑定~$\metav{x}$ 的量词的辖域内）。有了这个准备，我们的引入规则 $\exists$I 是：
\factoidbox{
...
这会是一个有缺陷的证明吗？如果确实有东西存在，那么我们当然可以从“所有东西都是$F$”这一事实，推断出“有东西是$F$”。但如果 \emph{根本没有任何} 东西存在呢？那么，“所有东西都是$F$”就显然是\emph{空真}的；然而，这并不能推出“有东西是$F$”，因为没有什么东西可以去 \emph{成为} $F$。所以，如果我们声称，仅凭逻辑，“$\exists x\,\atom{F}{x}$”就从“$\forall x\,\atom{F}{x}$”得出，那么我们就是在声称，仅凭\emph{逻辑}，就存在某物而非无物。这可能会让我们觉得有点奇怪。

事实上，我们已经默认了这个奇怪的前提。在\cref{s:FOLBuildingBlocks}中，我们规定，一阶逻辑中的论域必须至少有一个成员。然后，我们把（一阶逻辑的）有效性定义为一个在所有解释下都为真的语句。由于“$\exists x\, x=x$”将在每个解释下都为真，这\emph{同样}起到了规定作用：逻辑本身决定了存在某物而非无物。

既然逻辑是否应该告诉我们“必须有某物而非无物”这一点远非显而易见，我们在这里可能确实有点作弊的嫌疑。

然而，如果我们拒绝作弊，代价就很高。下面是我们想坚持的三个特性：

\begin{compactlist}
		\item $\forall x\,\atom{F}{x} \proves \atom{F}{a}$：毕竟，那就是 $\forall$E。
		\item $\atom{F}{a} \proves \exists x\,\atom{F}{x}$：毕竟，那就是 $\exists$I。
		\item 将证明复制粘贴以组合起来的能力：毕竟，推理就是通过将许多小步骤组合成相当长的链条来进行的。
	\end{compactlist}

如果我们在上述三个方面都如愿以偿，那么我们就必须承认$\forall x\,\atom{F}{x} \proves \exists x\,\atom{F}{x}$。因此，如果我们想要这三个特性，证明系统本身就告诉我们存在某物而非无物。而如果我们拒绝接受这一点，那么我们就得放弃我们想要坚持的三个特性之一！

在我们开始考虑放弃哪一个之前，我们不妨问问这个“作弊”究竟有多严重。诚然，它可能会使我们更难参与关于“为何存在某物而非无物”的神学辩论。但除此之外的大部分时间里，它都运行良好。所以，也许我们只需把我们的证明系统（以及更广义的一阶逻辑）看作具有一个非常略微受限的适用范围。如果我们将来想要允许\emph{无物存在}的可能性，那我们就不得不寻找一个更复杂的证明系统。但只要我们愿意忽略这种可能性，我们的证明系统就完全没问题。（同样，每个论域必须包含至少一个对象的规定也是如此。）

\section{全称引入}\label{s:forallIntro}

假设你已经证明了每一个特定的东西都是 F（并且没有其他东西需要考虑）。那么，你就有理由声称所有东西都是 F。这会启发我们得到以下证明规则。如果你已经建立了“$\forall x\,\atom{F}{x}$”的每一个代入实例，那么你就可以推断出“$\forall x\,\atom{F}{x}$”。

不幸的是，这条规则将完全无法使用。要建立每一个代入实例，就需要证明“$\atom{F}{a}$”、“$\atom{F}{b}$”、……“$\atom{F}{j_2}$”、……、“$\atom{F}{r_{79002}}$”、……，如此等等。事实上，由于一阶逻辑中有无穷多个名称，这个过程永无止境。所以我们永远无法应用那条规则。我们需要更巧妙一点，来制定我们的全称引入规则。

考虑下面这个论证会带给我们启发：
$$\forall x\,\atom{F}{x} \therefore \forall y\,\atom{F}{y}$$
这个论证应该是\emph{显然}有效的。毕竟，字母的选择应是随意的，不产生逻辑影响。但是我们的证明系统如何体现这一点呢？假设我们这样开始一个证明：
...
我们已经证明了“$\atom{F}{a}$”。当然，没有什么能阻止我们用同样的理由去证明“$\atom{F}{b}$”、“$\atom{F}{c}$”、……、“$\atom{F}{j_2}$”、……、“$\atom{F}{r_{79002}}$”……直到我们耗尽空间、时间或耐心。但仔细想想，我们发现，对于任何名称~\metav{c}，我们都有办法证明 $\atom{F}{\metav{c}}$。如果我们能对\emph{任何}东西做到这一点，我们当然应该说“$F$”对\emph{所有}东西都成立。因此，这证明了我们可以推断出“$\forall y\,\atom{F}{y}$”，像这样：
...
这里的关键想法是，“$a$”只是一个\emph{任意}的名称。它没有什么特别的——我们本可以选择任何其他名称——而且证明依然成立。这个关键想法构成了全称引入规则（$\forall$I）的动机：
\factoidbox{
...
\end{fitchproof}
其中，名称 \metav{c} 不得出现在任何前提或未解除的假设中。%\\
%\metav{x} must not occur in $\metav{A}(\ldots \metav{c} \ldots
%\metav{c}\ldots)$
}

在\cref{s:MainLogicalOperatorQuantifier}中，我们引入了一个用于指示\emph{代入实例}的约定：我们说，如果
$\metav{A}(\ldots \metav{x} \ldots \metav{x}\ldots)$ 是一个公式，那么 $\metav{A}(\ldots \metav{c} \ldots \metav{c}\ldots)$ 就是将所有 $\metav{x}$ 在 $\metav{A}$ 中的\emph{每一个}自由出现都替换为~$\metav{c}$ 的结果。为陈述 $\forall$I 规则，我们使用相同的约定，但方向相反：如果 $\metav{A}(\ldots \metav{c} \ldots \metav{c}\ldots)$ 是一个公式，那么 $\metav{A}(\ldots \metav{x} \ldots \metav{x}\ldots)$ 就是将其中\emph{每一个}~$\metav{c}$ 的出现都替换为~$\metav{x}$ 的结果。（仅当 $\metav{c}$ 在 $\metav{A}$ 中未出现于任何约束~$\metav{x}$ 的量词的辖域内时，才允许这样做。）

应用 $\forall$I 规则时有两点需要特别注意。若在选择名称~$\metav{c}$ 及将其替换为~$\metav{x}$ 时不加小心，则可能做出不正确的推理。

$\forall$I 规则的约束要求 \metav{c} 不得出现在任何前提或未解除的假设中。此约束确保我们总是在一个足够一般的层面上进行推理。%\footnote{Recall from \cref{s:BasicTFL} that we are treating “$\ered$” as a canonical contradiction. But if it were the canonical contradiction as involving some \emph{constant}, it might interfere with the constraint mentioned here. To avoid such problems, we will treat “$\ered$” as a canonical contradiction \emph{that involves no particular names}.}
为说明此约束的作用，考虑以下糟糕的论证：


\begin{earg}
		\item 人人都爱凯莉·米洛。
		\item[\texttherefore] 人人都爱他们自己。
	\end{earg}
我们可将这一明显无效的推理模式符号化为：
...
\begin{fitchproof}
	...
	\have{y}{\forall x\, \atom{L}{x,x}} \by{不当尝试应用 $\forall$I}{a}
\end{fitchproof}
这是不允许的，因为 “$k$’ 已出现在一个前提中（即第 $1$ 行）。关键在于，若我们对将要被概括的名称（其所指的对象）做出了任何（额外的）假设，则我们的推理就不足以一般到允许应用 $\forall$I。

虽然名称不得出现在任何\emph{未解除的}假设中，但它可以出现在\emph{已解除的}假设中，即它可以出现在一个已关闭的子证明内。例如，以下证明完全正确：
\begin{fitchproof}
	\open
		\hypo{f1}{\atom{G}{d}}\AS
...
\end{fitchproof}
这表明“$\forall z (\atom{G}{z} \eif \atom{G}{z})$”是一个\emph{定理}。而这正是应有的结果。

若我们只替换\emph{某些}名称而非全部，则会“证明”出荒谬的结论。例如，考虑以下论证：


\begin{earg}
	\item 每个人都和他们自己一样老。
	\item[\texttherefore] 每个人都和朱迪·丹奇一样老。
	\end{earg}
我们可将其符号化为：
$$\forall x\,\atom{O}{x,x} \therefore \forall x\,\atom{O}{x,d}$$
现假设我们试图用以下证明来确证这个糟糕的论证：
\begin{fitchproof}
	\hypo{x}{\forall x\, \atom{O}{x,x}}\PR
	\have{a}{\atom{O}{d,d}}\Ae{x}
	\have{y}{\forall x\, \atom{O}{x,d}}\by{不当尝试应用 $\forall$I}{a}
\end{fitchproof}
幸运的是，我们的规则不允许这样做：此证明尝试是禁止的，因为它并未将第 $2$ 行中\emph{每一个} “$d$’ 的出现都替换为 ”$x$’。

\section{存在消去}\label{s:existsElim}

假设我们知道\emph{某物}是~$F$。问题在于，仅知道这一点并未告诉我们哪个东西是~$F$。因此，从 “$\exists x\,\atom{F}{x}$” 出发，似乎不能直接推出 “$\atom{F}{a}$”、“$\atom{F}{e_{23}}$” 或该语句的任何其他代入实例。我们该如何做呢？

假设我们知道某物是~$F$，并且所有是~$F$ 的东西也都是~$G$。用（近乎）自然的英语，我们可以这样推理：


\begin{quote}
		既然有某物是 $F$，那么就存在某个特定的 $F$。我们除了知道它是 $F$ 外，对其一无所知，但为方便起见，不妨称它为“贝姬”。因此：贝姬是 $F$。由于所有是 $F$ 的东西都是~$G$，故贝姬是~$G$。既然贝姬是~$G$，则说明有某物是~$G$。并且此结论不依赖于贝姬具体是哪个对象。所以，有某物是~$G$。
	\end{quote}
我们可尝试在证明中如下刻画这一推理模式：
\begin{fitchproof}
...
\noindent
分解来看：我们首先写下假设。在第 $3$ 行，我们增加了一个额外假设：“$\atom{F}{b}$”。这只是 “$\exists x\,\atom{F}{x}$” 的一个代入实例。基于此假设，我们推出了 “$\exists x\,\atom{G}{x}$”。注意，对于名称 “$b$’ 所指的对象，我们并未做出任何\emph{特殊}假设；我们\emph{只}假设了它满足 ”$\atom{F}{x}$”。因此，推理不依赖于具体是哪个对象。而第 $1$ 行告诉我们\emph{有东西}满足 “$\atom{F}{x}$”，故我们的推理模式是完全一般的。我们可以解除具体的假设 “$\atom{F}{b}$”，而单独推出 “$\exists x\,\atom{G}{x}$”。

综上所述，我们得到存在消去规则 ($\exists$E)：
\factoidbox{
\begin{fitchproof}
...
与全称引入规则一样，这些约束条件至关重要。为理解其重要性，考虑以下糟糕的论证：
...
\fi
\end{fitchproof}
此证明的最后一行是不允许的。我们在第 $3$ 行为 “$\exists x\, \enot \atom{L}{x}$” 所用代入实例中的名称 “$b$’，出现在了第 $4$ 行中。下述尝试同样无效：
\begin{fitchproof}
	\hypo{f}{\atom{L}{b}}\PR
	\hypo{nf}{\exists x\, \enot \atom{L}{x}}\PR
	\open
...
\fi
\end{fitchproof}
最后一行仍然不允许。因为用于 “$\exists x\, \enot \atom{L}{x}$” 的代入实例中的名称 “$b$’，出现在了一个未解除的假设中（即第 $1$ 行）。
...
\fi
\end{fitchproof}
此处名称 “$a$’ 违反了 $\exists$E 规则的第二个条件：它出现在 ”$\exists x\, \atom{L}{a, x}$”（即第 $1$ 行）中。我们必须阻止这成为一个有效的证明，因为它是无效的：“有人喜欢自己”并不从“Alex喜欢某人”推出。

由此得出的要点是：\emph{若要从一个存在量词中提取信息，请为你的代入实例选择一个新的名称。}这样才能确保满足 $\exists$E 规则的所有约束。
...
\metav{c} \ldots \metav{c} \ldots)$ 是将 $\metav{A}$ 中 $\metav{x}$ 的所有\emph{自由}出现替换为~$\metav{c}$ 后得到的公式。此处我们强调“自由”是有原因的。它意味着你不能总是替换\emph{每一个}~$\metav{x}$ 的出现。例如，下述替换就是错误的：
...
在第 $2$ 行，我们错误地构造了代入实例：我们把 “$\atom{F}{x}$” 和 “$\atom{G}{x}$” 中的 “$x$” 都替换成了 “$a$”，但只有前一个 “$x$” 是自由出现---后一个被 “$\exists x$” 约束着。这不仅违反了关于替换的一条限制，还会使我们能够“证明”一个无效论证。由于 “$\exists x\,\atom{G}{a}$” 是一个空量化的例子，它等价于 “$\atom{G}{a}$”。此外，第 $1$ 行等价于 “$\forall x(\atom{F}{x} \land \exists y\,\atom{G}{y})$”。
不难看出，它不蕴涵 “$\atom{F}{a} \land
\atom{G}{a}$'。你不太可能遇到这样的情况（除非你的出题者别有用心）：你的练习题很可能只涉及像 “$\forall x(\atom{F}{x} \land \exists
y\,\atom{G}{y})$' 这样的句子，其中的变元被清晰地区分开。

在 \cref{s:existsIntro} 中，我们说过，用 “$\metav{A}(\ldots
\metav{x} \ldots \metav{c}\ldots)$' 表示通过以下方式得到的任何公式：将名称 \metav{c} 的一个或多个出现替换为变元~\metav{x}，\emph{前提是：$\metav{c}$ 在被替换的任何位置，都不位于一个约束着 $\metav{x}$ 的量词的辖域内}。这句强调的条件在某些情况下禁止替换 $\metav{c}$，这一点有些微妙。在实践中，你可能永远不会面临违反它的风险。但这一限制是保证规则正确性所必需的。例如，由于我们允许空量化，“$\exists
x\forall x\,\atom{L}{x,x}$' 是一阶逻辑的一个合式语句。只不过，其中的第一个 “$\exists x$” 并不起实际作用，该语句等价于 “$\forall x\,\atom{L}{x,x}$”。现在，“$\forall
x\,\atom{L}{x,x}$' \emph{本可以} 通过将名称~“$a$” 替换为变元~“$x$” 从 “$\forall
x\,\atom{L}{a,x}$' 得到，如果我们不仔细，忽略了 “$a$” \emph{确实} 位于 “$\forall x$” 的辖域内这一事实的话。如果没有上述限制，那么下面的推导就会被允许：
...
但由于 “$\exists x\, \forall x\, \atom{L}{x,x}$” 等价于 “$\forall x\, \atom{L}{x,x}$”，这将是一个无效的推理。为了明白为何如此，不妨把 “$L$” 解读为“喜欢”：从“Alex 喜欢每个人”并不能推出“每个人都喜欢每个人”。

类似地，在 \cref{s:forallIntro} 中，我们说过，$\metav{A}(\ldots
\metav{x} \ldots \metav{x}\ldots)$ 是将 $\metav{A}(\ldots \metav{c}
\ldots \metav{c}\ldots)$ 中 $\metav{c}$ 的\emph{每一个}出现替换为 $\metav{x}$ 的结果，但\emph{仅当 $\metav{c}$ 在 $\metav{A}$ 中不位于一个约束着 $\metav{x}$ 的量词的辖域内时才允许这样做}。如同 $\exists$I 规则陈述中包含的限制一样，在实践中你很少会面临违反它的风险。但这就是它为何重要。再次，由于我们允许空量化，“$\forall
x\exists x\,\atom{L}{x,x}$' 是一阶逻辑的一个合式语句。然而，“$\forall x\exists x\,\atom{L}{x,x}$” 可以通过将名称~“$a$” 处处替换为变元~“$x$” 从 “$\forall
x\,\atom{L}{a,x}$' 得到。这违反了我们所纳入的限制：在此例中，“$a$” 确实位于 “$\forall x$” 的辖域内。如果没有这个限制，下面的推导就会被允许：
...
但由于 “$\forall x\, \exists x\, \atom{L}{x,x}$” 等价于 “$\exists x\, \atom{L}{x,x}$”，这将是一个无效的推理：“有人喜欢他们自己”不能从“每个人都喜欢某个人”推出。

\practiceproblems
\problempart
解释为什么下面这两个“证明”是\emph{错误的}。同时，提供能证明这些“证明”所体现的谬误论证形式无效的解释：
...
下面的三个证明缺少引用（规则和行号）。请补充它们，使其成为完整的证明。
...
在\hyperref[pr.BarbaraEtc]{练习 \ref*{s:MoreMonadic}A}中，我们考察了亚里士多德逻辑的十五个三段论形式。请为每一种论证形式提供证明。注意：如果你将（例如）“没有F是G”符号化为“$\forall x (\atom{F}{x} \eif \enot \atom{G}{x})$”，你会发现证明要\emph{容易得多}。

\problempart
\label{pr.BarbaraEtc.proof2}
亚里士多德及其后继者还识别出其他一些依赖于“存在引入”的三段论形式。请用一阶逻辑符号化下述每一种论证形式，并给出证明。
...
\begin{compactlist}
	...
	\item \textbf{Camestros.} 有H。所有F都是G。没有H都是G。所以：有H不是F。
...
\begin{earg}
\item 有这样一个人，她喜欢所有喜欢每一个她所喜欢的人的人。
...
在\cref{s:stratTFL}中，我们讨论了使用真值函项逻辑的自然演绎基本规则来构造证明的策略。同样的原则也适用于量词规则。如果我们想证明一个量化句子$\forall \metav{x}\,
\atom{\metav{A}}{\metav{x}}$或$\exists \metav{x}\,
\atom{\metav{A}}{\metav{x}}$，我们可以进行逆向推理：试图通过$\forall$I或$\exists$I来证明目标句子，并尝试找到该规则对应前提的证明。而要从一个量化句子进行正向推理，我们则根据情况应用$\forall$E或$\exists$E。

具体来说，假设你想证明$\forall \metav{x}\, \atom{\metav{A}}{\metav{x}}$。要使用$\forall$I来证明，我们需要对某个名称~$\metav{c}$证明$\atom{\metav{A}}{\metav{c}}$，且该名称不能出现在任何未解除的假设中。应用相应的策略，即通过逆向推理构造$\forall \metav{x}\, \atom{\metav{A}}{\metav{x}}$的证明，做法是在其上方写下$\atom{\metav{A}}{\metav{c}}$，然后继续尝试寻找该句子的证明。
...
$\atom{\metav{A}}{\metav{c}}$是通过将$\atom{\metav{A}}{\metav{x}}$中$\metav{x}$的每个自由出现替换为~$\metav{c}$而从$\atom{\metav{A}}{\metav{x}}$得到的。为此，$\metav{c}$必须满足特殊条件。我们可以通过总是选择一个尚未在到目前为止已构造的证明中出现的名称来确保这一点。（当然，它最终会出现在我们构造的证明中——只是不在第~$n+1$行未解除的假设中。）

要从句子$\exists \metav{x}\, \atom{\metav{A}}{\metav{x}}$逆向推理，我们类似地在它上方写下一个句子，该句子可以作为$\exists$I规则应用的依据，即一个形式为$\atom{\metav{A}}{\metav{c}}$的句子。
...
这看起来就像我们从全称量化句子逆向推理时所做的。区别在于，对于$\forall$I，我们必须选择一个尚未在证明中（到目前为止）出现的名称~$\metav{c}$；而对于$\exists$I，我们可以且通常必须选择一个已经在证明中出现的名称~$\metav{c}$。就像在$\eor$I的情况下一样，通常不清楚哪个$\metav{c}$会奏效，因此为了避免不得不回溯，你应该只在所有其他策略都已应用后，才从存在量化句子进行逆向推理。

相比之下，从句子$\exists \metav{x}\, \atom{\metav{A}}{\metav{x}}$进行\emph{正向推理}通常总是有效的，且你无需回溯。从存在量化句子进行正向推理，不仅要考虑$\exists \metav{x}\, \atom{\metav{A}}{\metav{x}}$，还要考虑你希望证明的任何句子$\metav{B}$。它要求你在$\metav{B}$的上方设立一个子证明，其中$\metav{B}$是最后一行，并以$\exists \metav{x}\, \atom{\metav{A}}{\metav{x}}$的一个代入实例$\atom{\metav{A}}{\metav{c}}$作为假设。为了确保满足$\exists$E关于$\metav{c}$的条件，应选择一个尚未在证明中出现的名称$\metav{c}$。
...
随后你将继续以证明$\metav{B}$为目标，但现在是在一个子证明中，你有一个额外的句子可以运用，即~$\atom{\metav{A}}{\metav{c}}$。

最后，从$\forall \metav{x}\, \atom{\metav{A}}{\metav{x}}$进行正向推理意味着，对于任何名称~$\metav{c}$，你总是可以写下$\atom{\metav{A}}{\metav{c}}$并用$\forall$E证明它。当然，你不会想不加选择地这样做。只有某些名称$\metav{c}$会有助于你证明当前正在处理的目标句子。因此，就像从$\exists \metav{x}\, \atom{\metav{A}}{\metav{x}}$逆向推理一样，你应该只在所有其他策略都已应用后，才从$\forall \metav{x}\, \atom{\metav{A}}{\metav{x}}$进行正向推理。
...
真值函项逻辑中联结词的策略仍然适用，你应该以同样的顺序应用它们：首先从条件句、否定句、合取句以及（现在的）全称量化句逆向推理；然后从析取句以及（现在的）存在量化句正向推理；只有在这之后，才尝试应用$\eif$E、$\enot$E、$\lor$I、$\forall$E或$\exists$I。在我们的例子中，这意味着从结论逆向推理：
...
我们的下一步应该是从第$2$行的$\exists x\,\atom{A}{x}$进行正向推理。为此，我们必须选择一个尚未出现在证明中的名称。既然还没有任何名称出现，我们可以选任意一个，比方说“$d$”。
...
现在我们已用完了主要策略，应该从前提$\forall x(\atom{A}{x} \eif B)$进行正向推理了。应用$\forall$E意味着我们可以为$A(\metav{c}) \eif B$的任意实例提供证成，无论选择哪个$\metav{c}$。当然，选择$d$是明智的，因为这将给出$\atom{A}{d} \eif B$。然后我们就可以应用$\eif$E来证成$B$，从而完成证明。
...
注意，前提是一个条件句，而不是一个存在量化句子，因此我们（暂时）还不应该从它进行正向推理。从结论$\forall x(\atom{A}{x} \eif B)$逆向推理，引导我们去寻找$\atom{A}{d} \eif B$的证明：
...
现在我们可以从第$1$行的前提进行正向推理了。那是一个条件句，而它的后件恰好就是我们想要证成的句子$B$。所以我们应该寻找一个对其前件$\exists x\,\atom{A}{x}$的证明。当然，这现在是现成的，通过第$2$行的$\exists$I就能得到。这样我们就完成了：
...
这些证明需要使用间接证明。除基本量词规则外，仅使用真值函项逻辑的基本规则。

\chapter{量词的转换}\label{s:CQ}

在这一节，我们将在前一节基本规则的基础上增加一些补充规则。这些规则管辖量词和否定之间的互动。

在\cref{s:FOLBuildingBlocks}中，我们注意到$\enot\exists x\metav{A}$逻辑等价于$\forall x\, \enot\metav{A}$。我们将在证明系统中加入一些规则来管辖这一点。具体来说，我们增加：
	\factoidbox{
...
\problempart
在 \cref{s:MoreMonadic} 中，我们考虑了将量词“移过”各种逻辑算子时会发生什么。证明下列每对句子是可互推的：
...
注意：变元 “$x$” 在 “$\atom{G}{a}$” 中不出现。当一个句子的所有量词都出现在句首时，这个句子被称为处于 \emph{前束范式中}。这些等价式有时也被称为 \emph{前束化规则}，因为它们为我们提供了将任何句子化为前束范式的方法。

\chapter{同一性规则}
在 \cref{s:Interpretations} 中，我们提到了在哲学上有争议的论点，即\emph{不可分辨者的同一性}。它主张在所有方面都不可分辨的对象实际上是同一的。我们也提到，我们不会采纳这一论点。这意味着，无论你如何了解两个对象，都无法证明它们是同一的。当然，除非你本来就知道两者确实是同一的，但那样证明就没什么启发性了。

但核心要点是：\emph{任何}不含同一性谓词的句子，都无法证成“$a=b$”的推论。因此，我们的同一性引入规则不能允许我们推出包含两个\emph{不同}名称的同一性陈述。

然而，每个对象都与其自身同一。要断定某物与自身同一，无需任何前提。所以，同一性引入规则如下：
\factoidbox{
...
注意，这条规则无需引用证明中任何先前的行。对于任何名称 \metav{c}，你都可以在任何一行写下 $\metav{c}=\metav{c}$，并以 {=}I 规则作为唯一理据。

我们的消去规则则更有趣。如果你已确立“$a=b$”，那么任何对于以“$a$”命名的对象为真的东西，也必定对以“$b$”命名的对象为真。对于任何包含“$a$”的句子，你可以将其中的部分或全部“$a$”替换为“$b$”，从而得到一个等价的句子。例如，从“$\atom{R}{a,a}$”和“$a = b$”，你有理由推出“$\atom{R}{a,b}$”、“$\atom{R}{b,a}$”或“$\atom{R}{b,b}$”。更一般地：
\factoidbox{
...
这里的记法与 $\exists$I 规则类似。因此，$\metav{A}(\ldots \metav{a} \ldots \metav{a}\ldots)$ 是包含名称 $\metav{a}$ 的公式，而 $\metav{A}(\ldots \metav{b} \ldots \metav{a}\ldots)$ 则是通过将该公式中的一个或多个 $\metav{a}$ 实例替换为名称 $\metav{b}$ 而得到的公式。第 $m$ 行和第 $n$ 行出现的顺序不限，也不需相邻，但我们总是先引用同一性陈述。对称地，我们允许：
\factoidbox{
...
这条规则有时以 Gottfried Leibniz（戈特弗里德·莱布尼茨）之名，称为 \emph{Leibniz 律}。

为了演示这些规则，我们将证明几个简单的结果。首先，我们证明同一性是\emph{对称的}：
...
我们通过将第 $2$ 行中的一个“$a$”实例替换为“$b$”而得到第 $3$ 行；这是根据“$a= b$”而获得理据的。
...
我们通过将第 $3$ 行中的“$b$”替换为“$a$”而得到第 $4$ 行；这是根据“$a= b$”而获得理据的。

\practiceproblems
\problempart
\label{pr.identity}
为以下每个断言，提供一阶逻辑证明以表明其成立。
\begin{compactlist}
...
\end{itemize}
从而说明两者都有充分的理由用于符号化英语句子“Nick 是那个～$F$”。\\

\problempart
在\cref{sec.identity}中，我们声称以下公式都是英语句子“恰好存在一个 $F$”的逻辑等价符号化：
\begin{itemize}
...
\end{itemize}
证明它们都是可互推的。（\emph{提示}：要证明三个断言可互推，只需证明第一个可推出第二个、第二个可推出第三个、第三个可推出第一个即可；想想这是为什么吧。）
...
与真值函项逻辑的情况一样，我们首先将一阶逻辑的某些规则作为基本规则引入（在\cref{s:BasicFOL}中），然后又为量词转换添加了一些进一步规则（在\cref{s:CQ}中）。事实上，量词转换规则应被视为\emph{派生}规则，因为它们可以从\cref{s:BasicFOL}的\emph{基本}规则中推导出来。（这里的要点与\cref{s:Derived}相同。）以下是第一条量词转换规则的论证：
\begin{fitchproof}
...
\end{fitchproof}
这就解释了为什么量词转换规则可作为派生规则使用。对于另外两条量词转换规则，也可给出类似论证。
...
意味着没有哪个赋值（或解释）能使$\metav{A}_1, \metav{A}_2, \ldots, \metav{A}_n$全部为真而 $\metav{C}$ 为假。这关乎对语句的真值指派。它是一个\emph{语义概念}。

无论怎样强调这两个概念是不同的，都不过分。但我们还可以再强调一下：\emph{它们是不同的概念。}

一旦你内化了这一点，请继续往下读。

尽管我们的语义概念和证明论概念不同，但两者之间存在深刻的联系。为解释这种联系，我们将从思考有效式与定理的关系入手。

要证明一个语句是定理，只需构造一个证明。诚然，构造一个二十行的证明可能很难，但检查证明的每一行并确认其合法性却没那么难；如果证明的每一行本身都合法，那么整个证明就是合法的。然而，要证明一个语句是有效式，则需考虑所有可能的解释。在证明一个语句是定理和证明它是有效式之间，前者更容易。

反过来，要证明一个语句\emph{不是}定理则很难。我们需要考虑所有（可能）的证明。这非常困难。然而，要证明一个语句不是有效式，只需构造一个使其为假的解释。诚然，想出这样的解释可能很难；但一旦做到，检查它给语句指派了什么真值就相对直接了。在证明一个语句不是定理和证明它不是有效式之间，后者更容易。

幸运的是，\emph{一个语句是定理当且仅当它是逻辑有效式}。因此，如果我们给出了一个不依赖任何假设的 $\metav{A}$ 的证明，从而表明 $\metav{A}$ 是定理，即 ${}\proves \metav{A}$，我们便可以合理地推断 $\metav{A}$ 是逻辑有效式，即 $\entails\metav{A}$。类似地，如果我们构造了一个使 \metav{A} 为假的解释，从而表明它不是逻辑有效式，即 $\nentails \metav{A}$，那么就可得出 \metav{A} 不是定理，即 $\nproves \metav{A}$。
...
这表明，尽管可证性和语义蕴涵是\emph{不同的}概念，但它们在外延上是等价的。因此：
	\factoidbox{

\begin{factoiditems}
		\item 一个论证是\emph{有效的} \ifeff{} \emph{结论可以从前提中证明出来}。
		\item 一个语句是\emph{逻辑有效式} \ifeff{} 它是\emph{定理}。
		\item 两个语句是\emph{等价的} \ifeff{} 它们是\emph{可互推的}。
		\item 一组语句是\emph{共同可满足的} \ifeff{} 它们是\emph{共同一致的}。
	\end{factoiditems}

}
基于这个原因，你可以根据需要选择何时从证明的角度思考，何时从赋值/解释的角度思考，哪个方法对当前任务更容易就使用哪个。下一页的表格总结了哪种方法（通常）更容易。

可证性与语义蕴涵应当一致是直觉上合理的。但是——让我们再强调一遍——不要被符号“$\entails$”和“$\proves$”的相似性所蒙蔽。这两个符号的含义非常不同。可证性与语义蕴涵一致这一事实并非能轻易得到。

事实上，证明可证性与语义蕴涵相一致，非常明确地标志着一个关键转折点：从这里开始，入门逻辑进入了中级逻辑的范畴。
...
\begin{tabular*}{\textwidth}{p{.25\textheight}p{.325\textheight}p{.325\textheight}}
 & \textbf{是}  & \textbf{否}\\
\\
语句\metav{A}是否为\textbf{逻辑有效式}？
& 给出表明 $\proves\metav{A}$ 的证明
& 给出一个使 \metav{A} 为假的解释\\
\\
语句\metav{A}是否为\textbf{矛盾式}？ &
给出表明 $\metav{A} \proves \ered$ 的证明 &
给出一个使 \metav{A} 为真的解释\\
\\
...
%\\
语句 \metav{A} 和 \metav{B} 是否\textbf{等价}？ &
给出两个证明，一个证明 $\metav{A}\proves\metav{B}$，另一个证明 $\metav{B}\proves\metav{A}$
& 给出一个使 \metav{A} 和 \metav{B} 取不同真值的解释\\
\\
语句集 $\metav{A}_1, \metav{A}_2, \ldots, \metav{A}_n$ 是否\textbf{共同可满足}？
& 给出一个使所有 $\metav{A}_1, \metav{A}_2, \ldots, \metav{A}_n$ 为真的解释
& 从假设 $\metav{A}_1, \metav{A}_2, \ldots, \metav{A}_n$ 出发证明一个矛盾\\
\\
论证 $\metav{A}_1, \metav{A}_2, \ldots, \metav{A}_n \therefore \metav{C}$ 是否\textbf{有效}？
& 给出一个以 $\metav{A}_1, \metav{A}_2, \ldots, \metav{A}_n$ 为前提、以 \metav{C} 为结论的证明
& 给出一个使每个 $\metav{A}_1, \metav{A}_2, \ldots, \metav{A}_n$ 为真而 \metav{C} 为假的解释\\
\end{tabular*}
\end{center}
\ifHTMLtarget
\end{table}
