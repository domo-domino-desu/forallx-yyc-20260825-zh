...
\define{实质条件}。我们可以用条件联结词的特征真值表来总结这一点。
...
让我们来引入一个重要概念。
	\factoidbox{
		一个联结词是 \define{真值函项的} \ifeff{} 以该联结词为主逻辑算子的语句的真值，完全由构成它的成分语句的真值唯一决定。
	}
...
\section{构建完整真值表}
\define{完整真值表}为相关语句字母的每一个可能的真与假赋值提供了一行。每一行代表一个\emph{赋值}，而一个完整真值表为所有不同的赋值都提供了一行。
...
\section{重言式与矛盾式}
在\cref{s:BasicNotions}中，我们解释了\emph{必然真}与\emph{必然假}。这两个概念在真值函项逻辑中都有替代物。我们将从必然真的替代物开始。
	\factoidbox{
		$\metav{A}$ 是 \define{重言式} \ifeff{} 它在每个赋值下都为真。
	}
...
派生地，一组语句是\define{共同不可满足的}，当且仅当没有赋值能使它们全部为真。同样，使用真值表很容易检验共同可满足性。
