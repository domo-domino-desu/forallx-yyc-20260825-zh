...
\cref{s:IndicativeSubjunctive,s:ParadoxesOfMaterialConditional}中讨论一些精微之处。就现在而言，我们规定如下：$\metav{A}\eif\metav{B}$ 为假 \ifeff{} \metav{A} 为真且 \metav{B} 为假。具有这些规定性真值条件的联结词称为
...
让我们来引入一个重要概念。
	\factoidbox{
		一个联结词是\define{真值函项的（truth-functional）}\ifeff{} 以该联结词为主逻辑算子的语句的真值，完全由构成它的成分语句的真值唯一决定。
	}
...
虽然 $2 + 2 = 4$ 是必然情况，但肖斯塔科维奇写了十五部弦乐四重奏却并非\emph{必然}情况。如果肖斯塔科维奇去世得更早，他可能就无法完成第15号四重奏；如果他活得更久，他或许还会多写几部。因此，“情况必然是……”这个说法不是\emph{真值函项的}。
...
在\cref{s:CharacteristicTruthTables}中介绍实质条件句的特征真值表时，我们并没有给出任何理由来为其辩护。现在，让我们提供一个辩护，其思路遵循多萝西·埃金顿。\footnote{多萝西·埃金顿，“条件句”，《斯坦福哲学百科全书》（2020年秋季版）（\url{https://plato.stanford.edu/archives/fall2020/entries/conditionals/}）。}
...
\Cref*{brownwins1}是真的；\cref*{brownwins2}是假的，但两者的前件和后件都是假的。（希拉里没有赢；她没有成为美国第一位女总统；她也没有充满氦气飘走。）所以整个语句的真值并非由其各部分的真值唯一决定。

关键在于，\cref*{brownwins1,brownwins2}使用的是\emph{虚拟}条件句，而非\emph{直陈}条件句。它们让我们设想与事实相反的情况——毕竟希拉里·克林顿输掉了2016年大选——进而让我们评估在那种情况下\emph{会}发生什么。这类考虑根本无法用“$\eif$”来处理。
...
到目前为止，我们都是通过符号化约定\emph{间接地}为真值函项逻辑语句赋予真值。例如，我们可能会说，真值函项逻辑语句 “$B$” 为真 \ifeff{} 大本钟在伦敦。既然大本钟\emph{确实}在伦敦，这种符号化方式就会使 “$B$” 为真。但我们也可以\emph{直接地}指定真值。我们可以直接规定 “$B$” 为真，或者规定它为假。这类规定就称为\emph{赋值}：
	\factoidbox{
...
现在考虑子句 “$(H\eand I)$”。这是一个合取式，$(\metav{A}\eand\metav{B})$，其中 “$H$” 对应 \metav{A}，“$I$” 对应 \metav{B}。合取的特征真值表给出了\emph{任何}形如 $(\metav{A}\eand\metav{B})$ 的语句的真值条件，无论 $\metav{A}$ 和 $\metav{B}$ 是什么。它体现的要点是：一个合取式为真 \ifeff{} 它的两个合取支都为真。在这个例子里，我们的合取支就是 “$H$” 和 “$I$”。它们在真值表的第一行（且仅在第一行）同时为真。相应地，我们可以计算这个合取式在所有四行上的真值：
...
对于必然为假，我们也有一个类似的替代品：
	\factoidbox{
		$\metav{A}$是一个\define{矛盾式（contradiction）}（在真值函项逻辑中）\ifeff{} 它在每一个赋值下都为假。
	}
...
在\cref{s:BasicNotions}中我们说过，一组语句是共同可能的，当且仅当它们全部同时为真是可能的。我们也可以为这个概念提供一个替代品：
	\factoidbox{
		$\metav{A}_1, \metav{A}_2, \ldots, \metav{A}_n$是\define{共同可满足的（jointly satisfiable）}（在真值函项逻辑中）\ifeff{} 存在某个赋值使得它们全部为真。
	}
...
下面这个概念与共同可满足性密切相关：
	\factoidbox{
		语句 $\metav{A}_1, \metav{A}_2, \ldots, \metav{A}_n$\define{蕴涵（entail）}（在真值函项逻辑中）语句 $\metav{C}$ \ifeff{} 不存在任何对相关语句字母的赋值，使 $\metav{A}_1, \metav{A}_2, \ldots, \metav{A}_n$ 中的所有语句都真而 $\metav{C}$ 为假。
	}
...
注意：$\metav{A} \eif \metav{C}$ 是重言式 \ifeff{} 不存在任何对语句字母的赋值使得 $\metav{A} \eif \metav{C}$ 为假。由于条件仅在前件为真且后件为假时才为假，$\metav{A} \eif \metav{C}$ 是重言式 \ifeff{} 不存在任何赋值使得 $\metav{A}$ 为真而 $\metav{C}$ 为假。

结合这两个观察，我们看到 $\metav{A} \eif \metav{C}$ 是重言式 \ifeff{} $\metav{A} \entails \metav{C}$。但是，``$\entails$'' 和 ``$\eif$'' 之间有一个非常非常重要的区别：
	\factoidbox{


\begin{itemize}
			\item ``$\eif$'' 是真值函项逻辑的联结词。
			\item ``$\entails$'' 是扩展英语的符号。
		\end{itemize}
...
\begin{enumerate}
	\item\label{n:GodParadox} 并非如果上帝存在，上帝就会回应恶意的祈祷。
\end{enumerate}
...
在\cref{n:JanBald}中，我们同样遇到了一个例子，表明在真值函项逻辑中，直接的符号化并不完全适用。在真值函项逻辑中我们能做到的最好方案，是用它自己的语句字母，比如~`$N$'（这与我们的一贯做法相悖），来符号化 “Jan is not bald”。但这同样行不通。例如，我们就会把 “Jan is both bald and isn't” 符号化为 `$J \eand N$'。但 “Jan is both bald and isn't” 可以说是一个自相矛盾，而我们改写的符号 `$J \eand N$' 却并非如此。有些逻辑学家提出，对于像 “Jan is bald” 这样存在边界情况的句子，我们必须调整语义，并允许符号化它的语句字母~`$J$' 可以取真和假之外的其他真值。但这远未被普遍接受。

\Cref{n:GodParadox}同样表明，符号化常常是棘手的，对英语句子的直接符号化有时并不能捕捉到其预期的真值条件。但即使是我们改进后的\cref{n:GodParadox2}及其符号化 `$G \eif \enot M$'，结果也不够好。我们在这里遇到的现象，就是所谓的\emph{实质条件句悖论}之一。这个悖论在于，像 `$G \eif M$' 和 `$G \eif \enot M$' 这样的实质条件句，只要它们的前件~`$G$' 为假，它们就为真，尽管直觉上，“If God exists, She answers malevolent prayers”和“If God exists, She does\emph{not}answer malevolent prayers”这两句话不应同时为真。这表明，真值函项逻辑的联结词~`\eif' 并没有恰当地捕捉到这两个例子中的 “if \dots then”。真值函项逻辑的真值函项性在此处确实是一个局限。解决办法是将此处的 “if \dots then” 视为虚拟条件句（参见\cref{s:IndicativeSubjunctive}），但真值函项逻辑无法捕捉它们。\footnote{逻辑学家已经设计出能更好地处理虚拟条件句的逻辑系统。它们被称为“条件句逻辑”或“反事实逻辑”。}
