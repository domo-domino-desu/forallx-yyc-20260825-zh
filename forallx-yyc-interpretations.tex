...
Armed with this interpretation of `$\atom{H}{x}$', suppose we now add to our symbolization key:
	...
	\end{ekey}
Then `$\atom{H}{a}$' and `$\atom{H}{p}$' will both be true, on this interpretation, but `$\atom{H}{h}$' will be false, since Hypatia was not among the stipulated objects.
...
We defined a \define{valuation} in TFL as any assignment of truth and
falsity to sentence letters. In FOL, we are going to define an
\define{interpretation} as consisting of four
...
In our go-to interpretation, `$\exists x\, \atom{R}{b,x}$' should be
...
Suppose that \metav{A} is a formula. We will write
$$\metav{A}(\ldots \metav{x} \ldots \metav{x} \ldots)$$ to indicate
that \metav{A} contains some number of free occurrences of the
variable~\metav{x} (it is usually one or more, but we don't exclude
the case that \metav{x} does not occur free at all.) Suppose also that
\metav{c} is a name. Then we will write:
$$\metav{A}(\ldots \metav{c} \ldots \metav{c} \ldots)$$
for the formula we obtain by replacing \emph{every} free occurrence of
$\metav{x}$ in \metav{A} with~$\metav{c}$. The resulting formula is
called a \define{substitution instance} of $\forall
\metav{x}\metav{A}$ and $\exists\metav{x}\metav{A}$. Also, $\metav{c}$
is called the \define{instantiating name}. (If \metav{A} happens to
contain no free occurrence of~\metav{x}, then the substitution
instance is identical to~\metav{A}. So, for instance,
...
\factoidbox{
	\begin{factoiditems}
		\item The interpretation $\mathbf{I}[\text{d}/\metav{c}]$ is just like
		...
		$\metav{A}(\dots\metav{c}\dots\metav{c}\dots)$ is true in
		$\mathbf{I}[\text{d}/\metav{c}]$ (where the name $\metav{c}$ does not
