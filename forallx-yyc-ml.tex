...
这就是核心思想，但我们需要完善它，使之更精确。为此，我们需要引入模态逻辑\emph{解释}的概念。构建一个解释时，你首先需要包含一个\emph{可能世界}的集合。现在，说到这里你可能很想问：究竟什么是可能世界？直观的想法是，可能世界是这个世界本可以是的另一种样子。但这到底是什么意思呢？这是一个极好的哲学问题，我们稍后会详细探讨它。但眼下我们不需要为此过分担心。就形式逻辑而言，可能世界可以是你喜欢的任何东西。唯一重要的是，你给每个解释都提供一个非空的集合，其中的元素被标记为\define{可能世界}。
...
\begin{factoiditems}
		...
		\item $\metav{A}$ 是\define{模态永真式}\ifeff{} $\metav{A}$
		在所有解释的所有世界中都为真。
