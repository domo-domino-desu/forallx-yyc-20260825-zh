...
模态逻辑语义学背后的核心思想是这样的。在模态逻辑中，语句并非简单地非真即假。一个语句是\emph{在一个给定的可能世界中}真或假，而且同一个语句完全可以在一些世界中为真，在另一些世界中为假。然后我们说，$\ebox \metav{A}$ 为真当且仅当$\metav{A}$ 在\emph{所有}世界中为真；$\ediamond\metav{A}$ 为真当且仅当$\metav{A}$ 在\emph{某个}世界中为真。
...
\begin{compactlist}
	\item $\nu_w(\enot\metav{A})=T$当且仅当$\nu_w(\metav{A})=F$
	\item $\nu_w(\metav{A}\eand\metav{B})=T$当且仅当$\nu_w(\metav{A})=T$ 并且 $\nu_w(\metav{B})=T$
	\item $\nu_w(\metav{A}\eor\metav{B})=T$当且仅当$\nu_w(\metav{A})=T$ 或者 $\nu_w(\metav{B})=T$，或者两者都成立
	\item $\nu_w(\metav{A}\eif\metav{B})=T$当且仅当$\nu_w(\metav{A})=F$ 或者 $\nu_w(\metav{B})=T$，或者两者都成立
	\item $\nu_w(\metav{A}\eiff\metav{B})=T$当且仅当$\nu_w(\metav{A})=T$ 并且 $\nu_w(\metav{B})=T$，或者 $\nu_w(\metav{A})=F$ 并且 $\nu_w(\metav{B})=F$
\end{compactlist}
...
\begin{itemize}
	\item $\nu_w(\ebox \metav{A})=T$当且仅当$\forall w' (\nu_{w'}(\metav{A})=T)$
	\item $\nu_w(\ediamond \metav{A})=T$当且仅当$\exists w' (\nu_{w'}(\metav{A})=T)$
\end{itemize}
...
\begin{compactlist}[resume]
	\item $\nu_{w_1}(\ebox \metav{A})=T$当且仅当$\forall w_2 (Rw_1w_2\eif \nu_{w_2}(\metav{A})=T)$
	\item $\nu_{w_1}(\ediamond \metav{A})=T$当且仅当$\exists w_2 (Rw_1w_2\eand \nu_{w_2}(\metav{A})=T)$
\end{compactlist}

或者用平实的话说：$\ebox\metav{A}$ 在世界 $w_1$ 中为真当且仅当$\metav{A}$ 在每一个相对于 $w_1$ 可能的世界中都为真；而 $\ediamond\metav{A}$ 在世界 $w_1$ 中为真当且仅当$\metav{A}$ 在某个相对于 $w_1$ 可能的世界中为真。
...
\begin{factoiditems}
		...
		\metav{A}_n\therefore\metav{C}$ 是\define{模态有效的（modally valid）}当且仅当
		在所有解释的所有世界中，不存在
		$\metav{A}_1,\metav{A}_2, \dots \metav{A}_n$ 全部为真而
		$\metav{C}$ 为假的情况。

		\item $\metav{A}$ 是\define{模态永真式（modal truth）}当且仅当$\metav{A}$
		在所有解释的所有世界中都为真。

		\item $\metav{A}$ 是\define{模态矛盾式（modal contradiction）}当且仅当
		$\metav{A}$ 在所有解释的所有世界中都为假。

		\item $\metav{A}$ 是\define{模态可满足的（modally satisfiable）}当且仅当
...
\factoidbox{
	$\metav{A}_1,\metav{A}_2, \dots \metav{A}_n\entails_\mlT \metav{C}$当且仅当在任何\emph{具有自反可及关系的}解释中，都不存在一个世界，使得 $\metav{A}_1,\metav{A}_2, \dots \metav{A}_n$ 都为真而 $\metav{C}$ 为假
}
...
\factoidbox{
	$\metav{A}_1,\metav{A}_2, \dots \metav{A}_n\entails_\mlSfour \metav{C}$当且仅当在任何\emph{具有自反且传递的可及关系的}解释中，都不存在一个世界，使得 $\metav{A}_1,\metav{A}_2, \dots \metav{A}_n$ 都为真而 $\metav{C}$ 为假
}
...
\factoidbox{
	$\metav{A}_1,\metav{A}_2, \dots \metav{A}_n\entails_\mlSfive  \metav{C}$当且仅当在任何\emph{可及关系是等价关系的}解释中，都不存在一个世界，使得 $\metav{A}_1,\metav{A}_2, \dots \metav{A}_n$ 全部为真而 $\metav{C}$ 为假
}
...
\factoidbox{
	$\metav{A}_1,\metav{A}_2, \dots \metav{A}_n\entails_\mlSfive  \metav{C}$当且仅当在任何\emph{具有全通可及关系}的解释中，都不存在一个世界，使得 $\metav{A}_1,\metav{A}_2, \dots \metav{A}_n$ 全部为真而 $\metav{C}$ 为假。
}
